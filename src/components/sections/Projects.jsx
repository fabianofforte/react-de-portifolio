...
const Projects = () => {
  ...
  return (
    <section id="" className="relative py-20 bg-black overflow-hidden">
        ...
        <FadeIn delay={0}>
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-primary/10 border border-primary/30 rounded-full mb-6">
              ...
              <span className="text-sm text-primary font-medium">Minha Assinatura Digital</span>
            </div>
            ...
            {categories.map((category) => (
              <button key={category} onClick={() => handleCategoryChange(category)} className={`group relative px-6 py-3 rounded-full font-medium transition-all duration-300 ${activeCategory === category ? 'text-white' : 'text-white/60 hover:text-white'}`}>
                ...
                {activeCategory === category && (
                  <div className="absolute inset rounded-full bg-primary blur-xl opacity-50 -z-10" />
                )}
              </button>
            ))}
          </div>
